...
function resolveString(prior, incoming) {
  if (typeof prior === "string" && prior.trim()) return prior;
...
function normalizeSampleEntry(value) {
  ...
  const id =
    typeof value.id === "string"
      ...
      : null;
  const t =
    normalizeFiniteNumber(value.t) ??
    (value.t && typeof value.t.toMillis === "function" ? value.t.toMillis() : null);
  return { id, r, s, t };
}
...
async function aggregateSegmentPass({ admin, passRef, passData }) {
  ...
  const roughnessPercent =
    normalizeFiniteNumber(passData.roughnessPercent);
  ...
  const sampleCount =
    normalizeFiniteNumber(passData.sampleCount) && passData.sampleCount > 0
  ...
  const result = await admin.firestore().runTransaction(async (tx) => {
    ...
    const { sumR, sumS } = trimmedSamples.reduce(
      (acc, value) => {
    ...
    const existingPassesAllTime =
      normalizeFiniteNumber(existing.passesAllTime) ?? 0;
    const existingSamplesAllTime =
      normalizeFiniteNumber(existing.samplesAllTime) ?? 0;
    const newestSample = trimmedSamples[trimmedSamples.length - 1] || null;
    const lastAssessedAtValue =
      newestSample && newestSample.t
