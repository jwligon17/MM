...
const PatchRewardPage = ({
  ...
  const hasUnrevealed = useMemo(() => Boolean(unrevealedPatchImageSource), [unrevealedPatchImageSource]);

  const blurRadius = Platform.select({ ios: 25, android: 10, default: 20 });
  ...
  const runRevealAnimation = useCallback(() => {
    ...
    if (!revealHapticFiredRef.current && Platform.OS !== "web" && (grandReveal || hasUnrevealed)) {
  ...
  }, [grandReveal, glowProgress, hasUnrevealed, popScale, revealProgress, spinProgress, burstProgress, stopIdleGlow, stopIdlePulse]);
  ...
  }, [attached, grandReveal, hasUnrevealed, onAttachPatch, patchId, runCelebrateAnimation, runRevealAnimation, spinProgress]);
  ...
  }, [attached, grandReveal, glowProgress, hasUnrevealed, startIdleGlow, startIdlePulse, stopIdleGlow, stopIdlePulse]);

  useEffect(() => {
    if (!__DEV__) return;
    console.log("gray patch:", Image.resolveAssetSource(unrevealedPatchImageSource));
    console.log("color patch:", Image.resolveAssetSource(patchImageSource));
  }, [unrevealedPatchImageSource, patchImageSource]);
  ...
  }, [grandReveal, hasUnrevealed, isAttached, glowProgress, popScale, revealProgress, runRevealAnimation, spinProgress, burstProgress]);

  if (__DEV__) {
    console.log("[PatchRewardPage] hasUnrevealed=", hasUnrevealed);
    console.log("[PatchRewardPage] unrevealed=", Image.resolveAssetSource(unrevealedPatchImageSource));
    console.log("[PatchRewardPage] color=", Image.resolveAssetSource(patchImageSource));
  }
  ...
  return (
    <View style={styles.background}>
        ...
        {hasUnrevealed ? (
          <>
            <Animated.Image
              ...
              style={[StyleSheet.absoluteFillObject, { opacity: grayOpacity }]}
            />
            <Animated.Image
              source={patchImageSource}
              blurRadius={blurRadius}
              style={[StyleSheet.absoluteFillObject, { opacity: colorOpacity }]}
            />
          </>
        ) : (
          <ImageBackground
      ...
      <Animated.View pointerEvents="none" style={[styles.overlayWrapper, { opacity: overlayOpacity }]}>
        <LinearGradient
          pointerEvents="none"
          colors={[
            "rgba(0,0,0,0.78)",
          ...
          >
            <View style={[styles.patchShadow, attached && styles.patchShadowAttached]}>
              ...
              >
                ...
                <View style={styles.patchImageContainer} pointerEvents="none">
                  {hasUnrevealed ? (
...
const styles = StyleSheet.create({
  ...
  warmTint: {
    ...StyleSheet.absoluteFillObject,
    backgroundColor: "rgba(160,100,0,0.08)",
  },
  container: {
    flex: 1,
...
export default PatchRewardPage;
