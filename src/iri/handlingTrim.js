import { HANDLING_TRIM_MS } from "./constants";

/**
 * Marks samples invalid around "handling" periods using a timestamp-based buffer.
 * The state machine delays emission by trimMs so we can retroactively drop
 * samples from the lookback window when handling toggles on.
 */
export function createHandlingTrimmer({ trimMs = HANDLING_TRIM_MS } = {}) {
  ...
  const push = (sample, isHandling) => {
    if (!sample || typeof sample.timestampMs !== "number") {
      throw new Error("Sensor samples must include a numeric timestampMs");
    }
